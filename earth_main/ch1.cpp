//.cpp for displaying chapter 1 story
...
int ch1(){
    ...
    ofstream fout;
    string line;
    fin.open("ch1.txt"); //Displaying chapter one's story
    if (fin.fail()){
        cout<<RED<<"Chapter 1 cannot be displayed due to unknown issues, please try again!"<<RESET;
        ...
        while (getline(fin, line)){ //Read line by line, as well as detecting keywords to start game
            if (line=="Game starts!"){ //If detected keywords, start the corresponding game
                int pass=2, skip=0;
                cout<<GREEN<<"\bGame 1 music game starts!\n"
                "The game rule is very simple, there will be targets falling to the buttom\n"
                "You need to use 1 2 3 4 on your keyboard to hit the targets, better the timing higher the score you get!\n"
                "If you miss for three times you will lose and retry the game\n"
                "After earning score >= 200000, you win!\n"<<RESET;
                sleep(2);
    ...
    cout<<BLUE<<"\nYou have finished Chapter 1, do you want to continue or save and quit?\n"
    ...
    while (help==true){
        if (finish=="c"){
            status=1; //Game status, number means which chapter
            ...
            fout.close();
            ch2(); //The Next Chapter Starts Here
            help=false;
        }
        else if (finish=="q"){
            help=false;
