//.cpp for displaying chapter 3 story
...
int ch3(){
    ...
    ofstream fout;
    string line;
    fin.open("ch3.txt"); //Displaying chapter three's story
    if (fin.fail()){
        cout<<RED<<"Chapter 3 cannot be displayed due to unknown issues, please try again!"<<RESET;
        ...
        while (getline(fin, line)){ //Read line by line, as well as detecting keywords to start game
            if (line=="Game starts!"){ //If detected keywords, start the corresponding game
                cout<<RED<<"\nYou Must Maximize Your Terminal Size In Order To Play This Game!\nThe game rule is very simple: escape the maze.\nPress Any Key To Continue\n";
                cin.get();
                sleep(1);
                int pass=2, skip=0;
                while (pass!=1){
    ...
    cout<<BLUE<<"\nYou have finished Chapter 3, do you want to continue or save and quit?\n"
    ...
    while (help==true){
        if (finish=="c"){
            status=3; //Game status, number means which chapter
            ...
            fout.close();
            ch4(); //The Next Chapter Starts Here
            help=false;
        }
        else if (finish=="q"){
            help=false;
