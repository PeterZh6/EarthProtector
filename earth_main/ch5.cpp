//.cpp for displaying chapter 5 story
...
int ch5(){
    ...
    ofstream fout;
    string line;
    fin.open("ch5.txt"); //Displaying chapter five's story
    if (fin.fail()){
        cout<<RED<<"Chapter 5 cannot be displayed due to unknown issues, please try again!"<<RESET;
    ...
    cout<<BLUE<<"\nYou have finished Chapter 5, do you want to continue or save and quit?\n"
    ...
    while (help==true){
        if (finish=="c"){
            status=5; //Game status, number means which chapter
            ...
            fout.close();
            ch6(); //The Next Chapter Starts Here
            help=false;
        }
        else if (finish=="q"){
            help=false;
