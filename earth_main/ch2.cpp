//.cpp for displaying chapter 2 story
...
int ch2(){
    ...
    ofstream fout;
    string line;
    fin.open("ch2.txt"); //Displaying chapter two's story
    if (fin.fail()){
        cout<<RED<<"Chapter 2 cannot be displayed due to unknown issues, please try again!"<<RESET;
    ...
    cout<<BLUE<<"\nYou have finished Chapter 2, do you want to continue or save and quit?\n"
    ...
    while (help==true){
        if (finish=="c"){
            status=2; //Game status, number means which chapter
            ...
            fout.close();
            ch3(); //The Next Chapter Starts Here
            help=false;
        }
        else if (finish=="q"){
            help=false;
