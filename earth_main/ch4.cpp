//.cpp for displaying chapter 4 story
...
int ch4(){
    ...
    ofstream fout;
    string line;
    fin.open("ch4.txt"); //Displaying chapter four's story
    if (fin.fail()){
        cout<<RED<<"Chapter 4 cannot be displayed due to unknown issues, please try again!"<<RESET;
    ...
    cout<<BLUE<<"\nYou have finished Chapter 4, do you want to continue or save and quit?\n"
    ...
    while (help==true){
        if (finish=="c"){
            status=4; //Game status, number means which chapter
            ...
            fout.close();
            ch5(); //The Next Chapter Starts Here
            help=false;
        }
        else if (finish=="q"){
            help=false;
